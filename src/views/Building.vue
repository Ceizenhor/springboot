<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column prop="master" label="户主/责任人"></el-table-column>
      <el-table-column label="是否入住/驻" v-if="user.role === 'ROLE_ADMIN'" >
    ...
    <el-dialog title="楼宇信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="form" label-width="80px" size="small">
        ...
        <el-form-item label="户主/责任人" label-width="100px">
          <el-select clearable v-model="form.masterId" placeholder="请选择户主/责任人">
            <el-option  v-for="item in masters" :key="item.id" :label="item.nickname" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    load() {
      this.request.get("/building/page", {
        params: {
    ...
    save() {
      this.request.post("/building/", this.form).then(res => {
        if (res.code === '200') {
    ...
    changeEnable(row) {
      this.request.post("/building/update", row).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/building/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/building/del/batch", ids).then(res => {
        if (res.code === '200') {
